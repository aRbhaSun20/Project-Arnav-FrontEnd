...
function EditLocation({ openPopUp, setOpenPopup, selected }) {
  ...
  const handleSubmit = () => {
    if (imgSrc) {
      try {
        ...
        const imageRef = ref(storage, `${uuidV4()}.${getType(imgSrc.type)}`);
  ...
  const createLocation = async (dataUrl) => {
    try {
      const {
        ...
        query: `mutation editParent($_id: String, $placeName: String, $dataUrl: String) {
          editParent( _id:$_id ,parentName: $placeName, parentImageUrl:  $dataUrl) {
              _id
              parentName
          }
        }`,
        variables: { placeName, _id: selected?._id, dataUrl },
      });
      if (editParent) {
